...
ram_total=$(awk "BEGIN {printf \"%.2f\", $total / 1024}")
ram_used=$(awk "BEGIN {printf \"%.2f\", $used / 1024}")
ram_use_percent=$(awk "BEGIN {printf \"%.1f\", ($used / $total) * 100}")  # in %
...
ram_temp_average=$(awk "BEGIN {printf \"%.1f\", (${ram_temp_dimm1} + ${ram_temp_dimm2} + ${ram_temp_dimm3} + ${ram_temp_dimm4}) / 4}")

# Swap usage
read -r _ sw_total sw_used _ <<< $(free -m | awk '/^Swap:/ {print $1, $2, $3, $4}')
swap_total=$(awk "BEGIN {printf \"%.2f\", $sw_total / 1024}")
swap_used=$(awk "BEGIN {printf \"%.2f\", $sw_used / 1024}")
swap_use_percent=$(awk "BEGIN {printf \"%.1f\", ($sw_used / $sw_total) * 100}")  # in %
...
waybar_text="${memory_icon}  ${ram_use_percent}% / ${ram_temp_average}°C"
